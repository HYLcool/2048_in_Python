...
class Frame(wx.Frame):
	def __init__(self, title):
		...
		self.colors = {0: (204, 192, 179), 2: (238, 228, 218), 4: (237, 224, 200), 8:(242, 177, 121),
			...
			512: (237, 107, 114), 1024: (237, 207, 114), 2048: (237, 207, 114), 4096: (237, 207, 114),
			8192: (237, 207, 114), 16384: (237, 207, 114), 32768: (237, 207, 114), 65536: (237, 207, 114),
			131072: (237, 207, 114), 262144: (237, 207, 114), 524288: (237, 207, 114), 1048576: (237, 207, 114),
			2097152: (237, 207, 114), 4194304: (237, 207, 114), 8388608: (237, 207, 114), 16777216: (237, 207, 114),
			33554432: (237, 207, 114), 67108864: (237, 207, 114), 134217728: (237, 207, 114), 268435456: (237, 207, 114),
			536870912: (237, 207, 114), 1073741824: (237, 207, 114), 2147483648: (237, 207, 114), 4294967296: (237, 207, 114),
			8589934592: (237, 207, 114), 17179869184: (237, 207, 114), 34359738368: (237, 207, 114),
			68719476736: (237, 207, 114), 137438953472: (237, 207, 114), 274877906944: (237, 207, 114),
			549755813888: (237, 207, 114), 1099511627776: (237, 207, 114), 2199023255552: (237, 207, 114),
			4398046511104: (237, 207, 114), 8796093022208: (237, 207, 114), 17592186044416: (237, 207, 114),
			35184372088832: (237, 207, 114), 70368744177664: (237, 207, 114), 140737488355328: (237, 207, 114),
			281474976710656: (237, 207, 114), 562949953421312: (237, 207, 114), 1125899906842624: (237, 207, 114),
			2251799813685248: (237, 207, 114), 4503599627370496: (237, 207, 114), 9007199254740992: (237, 207, 114),
			18014398509481984: (237, 207, 114), 36028797018963968: (237, 207, 114), 72057594037927936: (237, 207, 114)}
	...
	def initGame(self):
		...
		count = 0
		# product 2 new blocks with 2 or 4
		while count < 2:
	...
	# when the window is changed, restart the initBuffer and drawAll
	...
	def drawLabel(self, dc):
		...
		dc.SetTextForeground((119, 110, 101))
		dc.DrawText(u"Merge the same numbers, and get 2048!", 15, 114)
		dc.DrawText(u"How to play:\n Use the up, down, left, right keys to move the blocks.\nWhen two same numbers crash, they will become one lager number!", 15, 639)
	...
	def drawScore(self, dc):
		...
		dc.DrawText(u"SCORE", 505 - 15 - bstScoreBoardW - curScoreBoardW + (curScoreBoardW - scoreLabelSize[0]) / 2, 48)
		dc.SetTextForeground((255, 255, 255))
		dc.DrawText(str(self.bstScore), 505 - 15 - bstScoreBoardW + (bstScoreBoardW - bestLabelSize[0]) / 2, 68)
		dc.DrawText(str(self.curScore), 505 - 15 - bstScoreBoardW - curScoreBoardW + (curScoreBoardW - scoreLabelSize[0]) / 2, 68)
	...
	# when the score is changed
   	...
   	def slideLeftRight(self, left):
   		...
   		for row in range(numRows):
   			cvl = [self.data[row][col] for col in range(numCols) if self.data[row][col] != 0]
   			if len(cvl) >= 2:
   				score += self.update(cvl, left)
   			for i in range(numCols - len(cvl)):
   				if left:
   					cvl.append(0)
   				else:
   					cvl.insert(0, 0)
   			for col in range(numCols):
   				self.data[row][col] = cvl[col]
   		return oldData != self.data, score
   	...
   	def doMove(self, move, score):
   		if move:
   			...
   			if self.isGameOver():
   				if wx.MessageBox(u"Game over, restart?", u"Game Over", wx.YES_NO | wx.ICON_INFORMATION) == wx.YES:
